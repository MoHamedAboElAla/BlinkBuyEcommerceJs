...
class Dashboard {
  ...
  //Add validation
   ...
   this.userEmailText = document.getElementById('userEmail');
   this.userEmailError = document.getElementById('userEmailError');
   this.userPassText = document.getElementById('userPassText');
   this.userPassError = document.getElementById('userPassError');
   this.userPasswordConfirm = document.getElementById("userPasswordConfirm"); // هذا لحقل التأكيد
   this.userPasswordConfirmError = document.getElementById("userPasswordConfirmError");

     this.usernameText.addEventListener('blur',function(e){
     // if ok skip else return error
     if(!isNameValid(this.value)){// if return false
      // here stop when input not skip
      this.select() ;// here i stop
 ...
 this.userEmailText.addEventListener('blur',function(e){
   // if ok skip else return error
   if(!isEmailValid(this.value)){// if return false
    // here stop when input not skip
    this.select() ;// here i stop
...
this.userPassText.addEventListener('blur',function(e){
 // if ok skip else return error
 if(!ispassValid(this.value)){// if return false
  // here stop when input not skip
  this.select() ;// here i stop
...
restrictFieldUser() {
    const fields = [
      ...
      this.userEmailText,
      this.userPassText,
      document.getElementById("userPasswordConfirm"),
    ...
    const validators = [
      ...
      isEmailValid,
      ispassValid,
      (val) => val === this.userPassText.value,
      () => true,
    ];
    ...
    fields.forEach((field, index) => {
      field.addEventListener("blur", () => {
        const isValid = validators[index](field.value);
        if (isValid && index + 1 < fields.length) {
          fields[index + 1].disabled = false;
        } else if (!isValid && index + 1 < fields.length) {
          fields[index + 1].disabled = true;
        }
      });
    });
  }
  setupValidation() {
    this.usernameText.addEventListener("blur", () => {
      if (!isNameValid(this.usernameText.value)) {
        this.usernameError.style.display = "block";
        this.usernameText.style.border = "2px solid red";
      } else {
        this.usernameError.style.display = "none";
        this.usernameText.style.border = "2px solid green";
      }
    });

    this.userEmailText.addEventListener("blur", () => {
      if (!isEmailValid(this.userEmailText.value)) {
        this.userEmailError.style.display = "block";
        this.userEmailText.style.border = "2px solid red";
      } else {
        this.userEmailError.style.display = "none";
        this.userEmailText.style.border = "2px solid green";
      }
    });

    this.userPassText.addEventListener("blur", () => {
      if (!ispassValid(this.userPassText.value)) {
        this.userPassError.style.display = "block";
        this.userPassText.style.border = "2px solid red";
      } else {
        this.userPassError.style.display = "none";
        this.userPassText.style.border = "2px solid green";
      }
    });

    this.userPasswordConfirm.addEventListener("blur", () => {
      if (this.userPasswordConfirm.value !== this.userPassText.value) {
        this.userPasswordConfirmError.style.display = "block";
        this.userPasswordConfirm.style.border = "2px solid red";
      } else {
        this.userPasswordConfirmError.style.display = "none";
        this.userPasswordConfirm.style.border = "2px solid green";
      }
    });
  }
...
async handleOrderSearch() {
  ...
  this.renderTable(filteredOrders, "order-table", ["id", "customerName", "products", "date", "total", "status"], [
   ...
   openAddUserModal() {
    ...
    document.getElementById("userPassText").value = "";
  ...
  async handleAddUserFormSubmit(event) {
   ...
   const nameInput = document.getElementById("userName");
    ...
    const passInput = document.getElementById("userPassText");
    ...
    const passError = document.getElementById("userPassError");
    const confirmError = document.getElementById("userPasswordConfirmError");
    ...
    if (pass.length < 6) {
      ...
      isValid = false;
      if (!firstInvalidField) firstInvalidField = passInput;
    }
    ...
    // Focus on the first invalid field (after showing all errors)
    if (!isValid) {
